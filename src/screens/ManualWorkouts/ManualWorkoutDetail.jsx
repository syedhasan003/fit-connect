...
export default function ManualWorkoutDetail() {
  ...
  days.forEach((day, dayIndex) => {
    // Use the saved day name if it exists and is meaningful, otherwise fall back to "Day N"
    const isGeneric = !day.name || /^day\s*\d+$/i.test(day.name.trim());
    const dayDisplayName = isGeneric ? `Day ${dayIndex + 1}` : day.name;
    ...
    <div style={{ minHeight: "100vh", background: "#000", color: "#fff", paddingBottom: "100px" }}>
      <div style={{ padding: "24px 20px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={handleTogglePin} style={{
                background: "transparent", border: "none", fontSize: 24, cursor: "pointer", padding: 8,
              }}>
                {workout.pinned ? "📌" : "📍"}
              </button>
              <button onClick={() => setShowDeleteModal(true)} style={{
                background: "transparent", border: "none", fontSize: 24,
                cursor: "pointer", padding: 8, color: "#ef4444",
              }}>
                🗑️
              </button>
            </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 12, marginBottom: 32 }}>
          <ActionButton icon="✏️" label="Edit Workout" color="#8b5cf6"
            onClick={handleEditWorkout} subtitle="Modify exercises and structure" />
          <ActionButton icon="▶" label="Start Workout" color="#3b82f6"
            onClick={handleStartWorkout} subtitle="Begin today's training session" />
          <ActionButton
        ...
        {/* EXERCISES */}
        <h2 style={{ fontSize: 19, fontWeight: 600, marginBottom: 16 }}>
  ...
  return (
    <button
      ...
      style={{
        ...
        background: disabled ? `${color}15` : isHover ? `${color}25` : `${color}15`,
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        ...
        {subtitle && (
          <div style={{ fontSize: 12, fontWeight: 400, color: `${color}cc`, lineHeight: 1.3 }}>
            {subtitle}
          </div>
        )}
      </div>
...
function ExerciseCard({ exercise, index }) {
  return (
    <div style={{
      ...
      <div style={{
        position: "absolute", inset: 0, borderRadius: 16, padding: "1px",
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <div>
            <h3 style={{ margin: 0, fontSize: 17, fontWeight: 600 }}>{exercise.name}</h3>
            {/* Shows real day name (e.g. "Monday") if assigned, otherwise "Day 1" */}
...
function DetailBadge({ label, value }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 13, fontWeight: 600, color: "#a78bfa" }}>{label}: {value}</span>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ width: 40, height: 4, background: "rgba(255,255,255,0.3)", borderRadius: 2, margin: "0 auto 24px" }} />
        <h3 style={{ margin: "0 0 12px", fontSize: 20, fontWeight: 600, color: "#fff" }}>Delete Workout?</h3>
...
function LoadingState() {
  return (
    <div style={{ minHeight: "100vh", background: "#000", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{
        width: 50, height: 50, border: "3px solid rgba(139,92,246,0.2)",
        borderTop: "3px solid #8b5cf6", borderRadius: "50%", animation: "spin 1s linear infinite",
      }} />
...
function ErrorState() {
  ...
  return (
    <div style={{
      ...
      justifyContent: "center", flexDirection: "column", gap: 16, color: "#fff", padding: 20,
    }}>
